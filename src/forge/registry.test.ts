...
import {
	...
	type ForgeDeps,
	resolveForge,
	resolveForgeFromConfig,
	resolveForgeKind,
	resolveForgeRegistry,
} from "./registry.ts";
...
describe("resolveForgeRegistry — warren-f012 multi-forge-support.md §2a", () => {
	describe("backward compat (no forges config)", () => {
		test("undefined forgesConfig → falls back to WARREN_FORGE env path", () => {
			const registry = resolveForgeRegistry(undefined, {});
			expect(registry.size).toBe(1);
		});

		test("undefined forgesConfig → single-entry keyed by resolved kind", () => {
			const registry = resolveForgeRegistry(undefined, { WARREN_FORGE: "fake" });
			expect(registry.has("fake")).toBe(true);
			expect(registry.get("fake")).toBeInstanceOf(FakeForge);
		});

		test("empty forgesConfig array → same as undefined (env-var fallback)", () => {
			const registry = resolveForgeRegistry([], { WARREN_FORGE: "fake" });
			expect(registry.size).toBe(1);
			expect(registry.has("fake")).toBe(true);
		});

		test("fallback key matches DEFAULT_FORGE_KIND when WARREN_FORGE unset", () => {
			const registry = resolveForgeRegistry(undefined, {});
			expect(registry.has(DEFAULT_FORGE_KIND)).toBe(true);
		});
	});

	describe("config-driven registry", () => {
		test("single fake entry builds a one-entry map keyed by id", () => {
			const registry = resolveForgeRegistry([{ id: "my-fake", kind: "fake" }], {});
			expect(registry.size).toBe(1);
			expect(registry.has("my-fake")).toBe(true);
			expect(registry.get("my-fake")).toBeInstanceOf(FakeForge);
		});

		test("multiple entries build the correct map", () => {
			const registry = resolveForgeRegistry(
				[
					{ id: "fake-1", kind: "fake" },
					{ id: "fake-2", kind: "fake" },
				],
			...
			expect(registry.has("fake-1")).toBe(true);
			expect(registry.has("fake-2")).toBe(true);
		});

		test("github entry resolves to GitHubForge", () => {
			const registry = resolveForgeRegistry(
				[{ id: "github", kind: "github", tokenEnv: "MY_GITHUB_TOKEN" }],
				{ MY_GITHUB_TOKEN: "test-pat" },
			);
			expect(registry.get("github")).toBeInstanceOf(GitHubForge);
		});

		test("github entry with missing tokenEnv var throws ForgeConfigError at boot", () => {
			expect(() =>
				resolveForgeRegistry([{ id: "github", kind: "github", tokenEnv: "MISSING_VAR" }], {}),
			).toThrow(ForgeConfigError);
		});

		test("ForgeConfigError for missing tokenEnv names the var and the forge id", () => {
			try {
				resolveForgeRegistry([{ id: "my-github", kind: "github", tokenEnv: "MISSING_TOKEN" }], {});
				throw new Error("unreachable");
			} catch (err) {
				expect(err).toBeInstanceOf(ForgeConfigError);
				const msg = (err as Error).message;
				expect(msg).toContain("MISSING_TOKEN");
				expect(msg).toContain("my-github");
			}
		});

		test("ForgeConfigError recovery hint names the var", () => {
			try {
				resolveForgeRegistry([{ id: "github", kind: "github", tokenEnv: "ABSENT_TOKEN" }], {});
				throw new Error("unreachable");
			} catch (err) {
				expect(err).toBeInstanceOf(ForgeConfigError);
				const hint = (err as ForgeConfigError).recoveryHint ?? "";
				expect(hint).toContain("ABSENT_TOKEN");
			}
		});

		test("github entry with empty tokenEnv value also throws", () => {
			expect(() =>
				resolveForgeRegistry([{ id: "github", kind: "github", tokenEnv: "EMPTY_TOKEN" }], {
					EMPTY_TOKEN: "",
				}),
			).toThrow(ForgeConfigError);
		});

		test("fake entry reads FAKE_FORGE_STATE_FILE when env var is set", async () => {
			const dir = await mkdtemp(join(tmpdir(), "warren-forge-test-"));
			const stateFile = join(dir, "fake-state.json");
			try {
				// FakeForge with a stateFile writes an empty JSON state on first read.
				// We just verify it constructs without error; the file is created lazily.
				const registry = resolveForgeRegistry([{ id: "fake", kind: "fake" }], {
					WARREN_FAKE_FORGE_STATE_FILE: stateFile,
				});
				expect(registry.get("fake")).toBeInstanceOf(FakeForge);
			} finally {
