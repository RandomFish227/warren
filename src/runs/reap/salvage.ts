...
export interface WorkspaceSalvageInput {
	...
	/**
	 * Per-spawn minted git credential for the rescue-ref push (warren-1154/4e1c,
	...
	/**
	 * HTTPS hostname of the git remote (e.g. `"github.com"`), extracted from
	 * the project's clone URL. Paired with `gitCredential` for the credential
	 * env. Omitted ⇒ no credential env (anonymous push).
	 */
...
export async function salvageWorkspace(
	...
	try {
		await input.exec.run("git", ["push", "origin", `HEAD:refs/heads/${branch}`], {
			...
			timeoutMs: 60_000,
			env: credentialGitEnv(input.gitCredential, input.originHost),
		});
		rescueRef = branch;
	} catch (err) {
